...
export default function App() {
  ...
  useEffect(() => {
    ...
    const restoreUser = async () => {
      ...
      u.getSession((err, session) => {
        ...
        u.getUserAttributes((err, attributes) => {
  ...
  const monthYear = new Date().toISOString().slice(0, 7);
    const res = await fetch(
      `${API}/expenses?user_id=${userId}&month_year=${monthYear}`
    );
  ...
  const fetchBudgets = async () => {
    ...
    const res = await fetch(`${API}/budgets?user_id=${userId}`);
    const data = await res.json();
  ...
  const handleDelete = async (id) => {
    ...
    const userId =
      user?.email ||
      user?.attributes?.email ;
      await fetch(`${API}/expense/${id}?user_id=${userId}`, {
        method: "DELETE",
      });
      fetchExpenses();
      window.dispatchEvent(new Event("expenseUpdated"));
  ...
  const handleSave = async () => {
    ...
    const userId =
      ...
      user?.attributes?.email;
      await fetch(
        `${API}/expense/${editData.expense_id}?user_id=${userId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
